...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  // part_buf를 선언
  char part_buf[128];  // 128 바이트 크기의 버퍼를 선언
  ...
  while (true) {
    ...
    if (!fb) {
      ...
      // JPEG 압축 확인
      if (fb->format != PIXFORMAT_JPEG) {
        bool jpeg_converted = frame2jpg(fb, 80, &jpg_buf, &jpg_buf_len); // JPEG 변환
        esp_camera_fb_return(fb);
        fb = NULL;
        if (!jpeg_converted) {
          log_e("JPEG compression failed");
    ...
    if (res == ESP_OK) {
      res = httpd_resp_send_chunk(req, _STREAM_BOUNDARY, strlen(_STREAM_BOUNDARY)); // Boundary 전송
    }
    if (res == ESP_OK) {
      size_t hlen = snprintf(part_buf, sizeof(part_buf), _STREAM_PART, jpg_buf_len, timestamp.tv_sec, timestamp.tv_usec);
      res = httpd_resp_send_chunk(req, part_buf, hlen); // 헤더 전송
    }
    if (res == ESP_OK) {
      res = httpd_resp_send_chunk(req, (const char *)jpg_buf, jpg_buf_len); // 이미지 본문 전송
    }
    if (fb) {
      esp_camera_fb_return(fb);
      fb = NULL;
    ...
    frame_time /= 1000;
    log_i("MJPG: %uB %ums (%.1ffps)", (uint32_t)(jpg_buf_len), (uint32_t)frame_time, 1000.0 / (uint32_t)frame_time);
  }
